...
class Crawler:
    ...
    def __init__(
        self,
        options: Dict[str, bool] = SCRAPER_DEFAULT_OPTIONS,
    ):
        logging.info(f"Initializing facebook_scraper with options: {options}")
        ...
        facebook_scraper._scraper.session.headers.update({"Accept-Language": "en-US"})

    def crawl_posts_from_group(
        self, group_id: Union[str, int], num_pages: int = 1
    ) -> List[Dict]:
        logging.info(f"Fetching {num_pages} pages of posts from group {group_id}")
    ...
    def crawl_posts_from_post_urls(self, post_urls: List[str]) -> List[Dict]:
        ...
        try:
            ...
            logging.error(f"Error fetching posts from urls {post_urls}: {e}")
            posts = []
        return posts
